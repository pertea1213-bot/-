...
- 어떤 키워드에도 안 걸리면 `notify.uncategorizedLabel`(기본값 "기타")로 묶여서 맨 뒤에 나옵니다.
- 지금 기본으로 들어있는 카테고리는 이호정님 서명에 있는 컨설팅 영역(R&D・자금・ESG・일생활균형・인사・조직・마케팅・수출・창업・재기・소공인・제조)을 기준으로 잡아뒀습니다. 필요하면 카테고리를 추가/삭제하거나 키워드를 조정하세요.
